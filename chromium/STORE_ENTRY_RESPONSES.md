# Chrome Web Store Entry Responses
...
The extension requires activeTab permission to capture screenshots of the current browser tab when users select CAD models displayed in web-based CAD applications. This permission is only activated when the user explicitly clicks the "Capture & Render" button, allowing the extension to access the visible content of the active tab to extract CAD screenshots for AI rendering. Without this permission, the extension cannot function as it needs to capture the visual content of CAD models displayed in the browser.

### desktopCapture justification
The desktopCapture permission is essential for the extension's core functionality. Users need to capture CAD models that may be displayed in desktop applications (not just browser tabs), such as AutoCAD, SolidWorks, or other CAD software. When users click "Capture & Render", they can select any area of their screen, including desktop applications, to capture CAD wireframes for photorealistic rendering. This permission is only used when the user explicitly initiates a capture action and is required to provide the full-screen selection tool that allows precise area selection.
